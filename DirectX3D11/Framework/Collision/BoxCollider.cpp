...
bool BoxCollider::SphereCollision(SphereCollider* collider)
{
	...
	Vector3 closestPos = box.position;
	Vector3 spherePos = collider->GlobalPos();

	for (UINT i = 0; i < 3; i++)
	{
		float length = Vector3::Dot(box.axis[i], spherePos - box.position);

		float mult = (length < 0.0f) ? -1.0f : 1.0f;

		length = min(abs(length), box.halfSize[i]);

		closestPos += box.axis[i] * length * mult;
	}

	float distance = Distance(spherePos, closestPos);
