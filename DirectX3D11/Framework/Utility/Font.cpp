...
{
	int len = WideCharToMultiByte(CP_ACP, 0, &value[0], -1,
		nullptr, 0, nullptr, nullptr);
	string strMulti(len, 0);
	WideCharToMultiByte(CP_ACP, 0, &value[0], -1,
		&strMulti[0], len, nullptr, nullptr);
...
{
	int nLen = MultiByteToWideChar(CP_ACP, 0, &value[0], value.size(),
		nullptr, 0);
	wstring strUni(nLen + 1, 0);
	int len = MultiByteToWideChar(CP_ACP, 0, value.c_str(), value.size(),
		&strUni[0], nLen);
	strUni[len] = 0;
